...
acceptance and remains published as historical evidence.  `v1.0.6-rc.5` is the
current acceptance candidate after publication.  It adds HTTPS companion
service and adapter support while retaining the rc.4 timing behavior.  Version `1.0.6` adds the one-line convenience runner, its
...
After rc.5 is published, prerelease acceptance uses
...
for example `v1.0.6` or `v1.0.6-rc.5`.  Anything else, including whitespace,
...
3. Build and validate the immutable bundle, commit it, render the bootstrap
   ...
   release commit.
4. Publish the current release candidate, `v1.0.6-rc.5` for this release,
...
- prerelease: `--confirm 'PUBLISH PRERELEASE v1.0.6-rc.5'`, published with
  `--prerelease --latest=false`;
- production: `--kind production --tag v1.0.6 --accepted-prerelease
  v1.0.6-rc.5 --confirm 'PUBLISH PRODUCTION v1.0.6'`, published with
  `--latest`.
...
- version `1.0.6` `install-all`, `uninstall-all`, and one-line runner
  acceptance on Raspberry Pi 3 against the `v1.0.6-rc.5` prerelease, followed
